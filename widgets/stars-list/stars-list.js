...
let bem = utils.bemHelper('ais-stars-list');
 ...
 * @param  {string} options.facetName Name of the attribute for filtering
...
function starsList({
  ...
  let usage = 'Usage: starsList({container, facetName, max, [cssClasses.{root,header,body,footer,list,item,active,link,disabledLink,star,emptyStar,count}, templates.{header,item,footer}, labels.{andUp}, transformData, autoHideContainer]})';
  ...
  return {
    ...
    render: function({helper, results, templatesConfig, state, createURL}) {
      ...
      results.getFacetValues(facetName).forEach(facet => {
      ...
      ReactDOM.render(
        <RefinementList
          createURL={(stars) => {
            return createURL(state.toggleRefinement(facetName, stars));
          }}
    ...
    _toggleRefinement: function(helper, facetValue) {
      ...
      if (!isRefined) {
        for (let val = +facetValue; val <= max; ++val) {
          helper.addDisjunctiveFacetRefinement(facetName, val);
        }
      }
      helper.search();
    },
